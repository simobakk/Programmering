...
# Paths
mcclear_path = '/måledata/temp/mcclear_søråsjordet_2023.csv'
...
current_file_path = '/'
dir_file_path = current_file_path + '/måledata/søråsjordet'
for filename in os.listdir(dir_file_path):
    if filename.endswith('.dat'):
        ...
        max_dni = merged_test_df['dni_clear'].max()
        buffer = 0.1
        ax.set_ylim(0, max(100,max(max(max_dni,max_cs), max_ir)) * (1 + buffer))
        ...
        for day in range(len(unique_days)):
            ...
            # filtered_measured = sj_11223_df[sj_11223_df['TIMESTAMP'].dt.date == day]
            ...
            dni.set_data(merged_test_df['TIME'][0::60], merged_test_df['dni_clear'][0::60])
